import audioController from "../domain/audioController.js";
...
const player = {
  ...
  eventListeners() {
    this._audioController._controller.addEventListener(
      "timeupdate",
      function () {
        progress_value =
          (player._audioController.currentTime /
            player._audioController.duration) *
          100;
    ...
    this._progress_bar.addEventListener("input", function () {
      player._audioController._controller.currentTime =
        (this.value / 100) * player._audioController._controller.duration;
    });
    this._forwd_btn.addEventListener("click", function () {
      audioController.nextSong();
      player._interface.loadSong(player._audioController._actualSong);
    });
